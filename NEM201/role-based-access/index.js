...
app.post('/createlecture', async (req, res) => {
    ...
    try {
        ...
        if (role !== 'instructor') {
            return res
                .status(403)
                .send('You are not allowed to create lecture');
        } else {
            // create a lecture, store into db
...
// if admin signups , it will create instructor
// else student is created
app.post('/signup', async (req, res) => {
    ...
    try {
        ...
        return res.send('Non admin user tried to create an instructor account');
    }

    const user = new UserModel({ name, email, password: hash, age });
    await user.save();
    return res.send(201).send('Student created successfully');
});
...
app.post('signup/instructor', async (req, res) => {
    ...
    await user.save();
    return res.send(201).send('Instructor created successfully');
});
